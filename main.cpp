...
struct HPMPluginInstance {
    ...
    HPMPluginInstance(io_service_t service) {
        SInt32 score;
        IOReturn ret = IOCreatePlugInInterfaceForService(service, kAppleHPMLibType, kIOCFPlugInInterfaceID, &plugin, &score);
        if (ret != kIOReturnSuccess)
            throw failure("IOCreatePlugInInterfaceForService failed");

        HRESULT res = (*plugin)->QueryInterface(plugin, CFUUIDGetUUIDBytes(kAppleHPMLibInterface), (LPVOID *) &device);
        if (res != S_OK)
...
std::unique_ptr<HPMPluginInstance> FindDevice() {
    ...
    while ((device = IOIteratorNext(iter))) {
        ...
        CFNumberRef data;
        int32_t rid;
        data = (CFNumberRef)IORegistryEntryCreateCFProperty(device,
                                               CFSTR("RID"),
